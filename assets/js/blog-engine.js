(function () {
  function sitePath(path) {
    ...
    if (window.location.protocol === 'file:') {
      const scriptSrc = document.currentScript?.src || Array.from(document.scripts)
        .map((s) => s.src)
        .find((src) => /\/assets\/js\/blog-engine\.js(?:\?|$)/.test(src));
      const base = scriptSrc ? scriptSrc.replace(/\/assets\/js\/blog-engine\.js(?:\?.*)?$/, '/') : window.location.href;
